...
MIN_HOUR_CLICKS        = 10   # skip hour-placement cohorts with fewer clicks than this threshold
...
# ── Query ──────────────────────────────────────────────────────────────────────
# Hour bucket is derived from click event_timestamp in Python.
QUERY = """
WITH winners AS (
    SELECT
        acd.auction_id,
...
# Used to compute before/after ROAS per (hour_bucket, placement group) cohort.
ROAS_QUERY = """
SELECT
    CAMPAIGN_ID,
...
def plot_heatmaps(summary: pd.DataFrame, event_date: str = EVENT_DATE) -> None:
    pivot_lift  = summary.pivot(index="hour_bucket", columns="placement_group", values="total_lift_pct")
    pivot_delta = summary.pivot(index="hour_bucket", columns="placement_group", values="best_delta")

    # order rows by hour (0–23)
    row_order   = sorted(pivot_lift.index)
    ...
    fig.suptitle(
        f"Revenue Lift by Hour Bucket & Placement Group\n(SP clicked winners, budget-aware, {event_date})",
        fontsize=15,
    )
...
def plot_roas_heatmaps(summary: pd.DataFrame, event_date: str = EVENT_DATE) -> None:
    """
    Three side-by-side ROAS heatmaps per (hour_bucket, placement group):
    ...
    """
    pivot_before = summary.pivot(index="hour_bucket", columns="placement_group", values="roas_before")
    pivot_after  = summary.pivot(index="hour_bucket", columns="placement_group", values="roas_after")
    pivot_change = summary.pivot(index="hour_bucket", columns="placement_group", values="roas_change")

    # order rows by hour (0–23)
    row_order    = sorted(pivot_before.index.union(pivot_after.index).union(pivot_change.index))
...
df = fetch_data()
df.to_pickle("../data/segment_placement_hour_bucket_revenue_df.pkl")

# df = pd.read_pickle("../data/segment_placement_hour_bucket_revenue_df.pkl")
...
roas_df = fetch_roas()
roas_df.to_pickle("../data/segment_placement_hour_bucket_roas_df.pkl")

# roas_df = pd.read_pickle("../data/segment_placement_hour_bucket_roas_df.pkl")
print(f"  Campaigns with ROAS data: {len(roas_df):,}")

#%%
print(f"\nFetching campaign daily budgets for {EVENT_DATE}...")
budget_df = fetch_budget()
budget_df.to_pickle("../data/segment_placement_hour_bucket_budget_df.pkl")

# budget_df = pd.read_pickle("../data/segment_placement_hour_bucket_budget_df.pkl")
...
# Derive hour bucket (0–23) from click timestamp
df["hour_bucket"] = df["event_timestamp"].dt.hour

# Enumerate cohorts with enough clicks
cohorts = (
    df.groupby(["hour_bucket", "placement_group"])
    .size()
    .reset_index(name="n_rows")
    .query("n_rows >= @MIN_HOUR_CLICKS")
    .sort_values(["hour_bucket", "placement_group"])
    .reset_index(drop=True)
)
print(f"\n  Cohorts with >= {MIN_HOUR_CLICKS} clicks: {len(cohorts)}")

#%%
summary_rows = []
for _, cohort in cohorts.iterrows():
    hb = cohort["hour_bucket"]
    pg = cohort["placement_group"]
    df_seg = df[(df["hour_bucket"] == hb) & (df["placement_group"] == pg)]
    ...
    print(f"  [Hour {hb:02d} / {pg}]  best Δ=${best['delta']:.2f}  lift={best['total_lift_pct']:.4f}%")
...
print("\nRevenue Lift by (Hour Bucket, Placement Group) — sorted by total lift:")
print(f"{'Hour':<6} {'Placement Group':<15} {'Rows':>8} {'Best Δ ($)':>12} {'Total Lift (%)':>16}")
print("-" * 63)
for _, row in summary.iterrows():
    print(
        f"{row['hour_bucket']:<6} {row['placement_group']:<15}"
...
for _, row in summary.iterrows():
    ...
    cohort_df = df[(df["hour_bucket"] == hb) & (df["placement_group"] == pg)]
...
summary = summary.merge(roas_summary, on=["hour_bucket", "placement_group"], how="left")

print("\nROAS Before vs After (best Δ per cohort):")
print(f"{'Hour':<6} {'Placement Group':<15} {'Best Δ ($)':>12} {'ROAS Before':>13} {'ROAS After':>12} {'ROAS Δ':>10}")
print("-" * 73)
...
def plot_cpc_heatmaps(summary: pd.DataFrame, event_date: str = EVENT_DATE) -> None:
    """
    Three side-by-side avg-CPC heatmaps per (hour_bucket, placement group):
    ...
    pivot_before = df.pivot(index="hour_bucket", columns="placement_group", values="avg_cpc_before")
    pivot_after  = df.pivot(index="hour_bucket", columns="placement_group", values="avg_cpc_after")
    pivot_delta  = df.pivot(index="hour_bucket", columns="placement_group", values="avg_cpc_delta")

    # order rows by hour (0–23)
    row_order    = sorted(pivot_before.index.union(pivot_after.index).union(pivot_delta.index))
    pivot_before = pivot_before.reindex(row_order)
    pivot_after  = pivot_after.reindex(row_order)
    pivot_delta  = pivot_delta.reindex(row_order)
...
#%%
# ── Top 2 hour buckets by revenue lift: lift curve per placement group ─────────
SEGMENT_MAX_DELTA = 2.0
top_hours = (
    summary.groupby("hour_bucket")["total_lift_pct"]
    .max()
    .nlargest(2)
    .index
    .tolist()
...
segment_specs = [
    (f"Hour {hb:02d}", cohorts[cohorts["hour_bucket"] == hb])
    for hb in top_hours
]
...
for ax, (label, seg_cohorts) in zip(axes, segment_specs):
    for _, cohort in seg_cohorts.iterrows():
        pg = cohort["placement_group"]
        df_seg = df[(df["hour_bucket"] == cohort["hour_bucket"]) & (df["placement_group"] == pg)]
...
#%%
# ── Debug: per-placement breakdown for given hour buckets ─────────────────────
def debug_cohort(hour_buckets: list) -> None:
    """
    For each hour bucket in hour_buckets, print a per-placement breakdown:
      - Cohort sales ($)
      - Cohort ad fee before / after hard reserve increment ($)
      - ROAS before / after
      - Avg CPC before / after ($)
      - Cohort clicks (n_rows)
    """
    ...
    for hb in hour_buckets:
        rows = summary[summary["hour_bucket"] == hb].sort_values("placement_group")
        if rows.empty:
            print(f"\nNo data for hour: {hb}")
            continue

        print(f"\n{'=' * len(cols)}")
        print(f"Hour Bucket: {hb:02d}")
        print(cols)
        print("-" * len(cols))
